...
| Migración mock → Neon (siguientes indicadores) | Faltan por migrar: `Outgoing Calls`, `Performance`, `Productivity`, `Ranking`, `Bonus`, `Backlog Team` (legacy mock; no confundir con el `Backlog` CAC real). Los KPI cards superiores también siguen en mock. Misma técnica: agregar al `Set DB_INDICATORS` + extender `dbTrendByBucket` |
| Mapeo completo de joins en Roster | Filas del Google Sheet con nombre real en `Compass` / `CallPicker` / `QA` / `Genesys` en vez del identificador `User X` reciben 0 datos del indicador correspondiente hasta que se complete el mapeo |
| Day-inclusive margin en `/api/opened-cases` | El backend agrega ±1 día al filtro (tolerancia de timezone). Puede devolver casos un día fuera del rango seleccionado |
...
- `dbTrendByBucket` retorna `{ out, qaByBucket, nsatByBucket, nsatInfoByBucket, nsatClaimsByBucket, backlogByBucket, fcrByBucket, callsEffByBucket, productivityByBucket }`:
  - `out` — buckets con valores tipo COUNT/SUM/SNAPSHOT (0 es legítimo). Incluye `Opened Cases` (de `Abiertos`), `Closed Cases` (de `Cerrados`), `Incoming Calls`, `Still Open Cases` y cuatro contadores de desglose: `Opened Cases Information`, `Opened Cases Complaint` (filtrados por `contact_reason_1` de `Abiertos`), `Closed Cases Information`, `Closed Cases Complaint` (filtrados por `contact_reason_1` de `Cerrados`). Los componentes crudos `Closed Cases` / `Incoming Calls` para Calls Efficiency se re-leen vía `_callsEffClosed` / `_callsEffCalls` en el tooltip.
  ...
  - `callsEffByBucket` — `(Closed Cases / Incoming Calls) × 100` por bucket; buckets con 0 llamadas omitidos → gap.
  - `productivityByBucket` — varía por `currentUser.subNivel`: Calls = `(CallsEff×0.5)+(QAnorm×0.5)`; Follow up = `(CasesEff×0.5)+(QAnorm×0.5)`; NA → mock.
...
- **`indicatorSummary`** — useMemo que produce totales del periodo por indicador como badges en la esquina superior derecha del Paper del line chart. Hasta 3 badges: teal `#0ba0af` (idx 0), purple `#B018D9` (idx 1), orange `#FF7A00` (idx 2). COUNT_SET → suma; NSAT_SET → NPS index; PCT_SET → promedio. `null` → badge oculto.
- Al agregar un indicador nuevo tocar: `indicatorOptions`, `CasesTooltip` (si necesita sub-filas), `memberBuckets`, `memberValuesForRanking`, `teamValueOf` en `aggregatedTrendData`, e `indicatorSummary` (clasificar en COUNT_SET / NSAT_SET / PCT_SET).
