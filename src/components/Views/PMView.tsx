...
// Types, PHASE_DEFS, PHASE_COLORS imported from ../../data/pepsicoMockData
...
// PHASE_DEFS, PHASE_COLORS, Campaign types imported from ../../data/pepsicoMockData
...
// HierarchyKey imported from pepsicoMockData
...
const PMView: React.FC<{ member?: any }> = ({ member }) => {
  ...
  const { performance: myPerformance } = metrics;

  // Derive quartile from performance for the KPI card banner
  const myQuartile =
    myPerformance >= 87 ? 'Q1' : myPerformance >= 75 ? 'Q2' : myPerformance >= 62 ? 'Q3' : 'Q4';

  // Ranking percentile seeded from RFC (visual only — PM doesn't know team data)
  const rankingPercentile = activeRfc
    ? 5 + Math.floor((Math.sin(activeRfc.length * 17.3 + 42) * 0.5 + 0.5) * 30)
    : 18;

  const campaignsByPhase = PHASE_DEFS.map(def => ({
    name: `${String(def.number).padStart(2, '0')} ${def.name}`,
    value: campaigns.filter(c => currentPhaseOf(c).number === def.number).length,
    color: def.color,
  }));
      ...
      <Box sx={{ height: '24%', display: 'flex', gap: 2, flexShrink: 0 }}>
        <Box sx={{ flex: 1 }}>
          <ManagementIndicator
            ...
            color={getOnTimeColor(myPerformance)}
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1, minHeight: 0 }}>
        ...
        <Box sx={{ flex: 1, display: 'flex', gap: 2, minHeight: 0 }}>
          ...
          >
            ...
            <Box sx={{ flex: 1, minHeight: 0 }}>
              <GanttChart campaigns={campaigns} brandFilter={brandFilter} categoryFilter={categoryFilter} statusFilter={statusFilter} />
            </Box>
          </Paper>
